...
import json
import pygame
from arenbels.display.engine.automata import *
...
class SpriteScheduler:
	...
	def load_automaton(self):
		...
		if self.ata is None:
			logging.warning("Initialization Failed ! name:"+str(self.name),"\n")
	...
	def step(self,char):
		...
		try:
			...
			logging.debug("No transition from state "+str(self.ata.cs)+" with letter "+char+".")
	...
	def load_sprites(self):
		...
		if not self.loaded:
			...
			for k,v in self.ata.qn.items():
				if type(v) == type(""):
					self.ata.qn[k] = pygame.image.load("arenbels/data/img/" + v)
			...
			logging.warning("WARNING : You are trying to load already loaded images!")
