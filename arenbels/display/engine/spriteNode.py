from arenbels.display.engine.node import Node
from pygame.transform import smoothscale
from arenbels.display.engine.vector import Vector
from arenbels.display.engine.rect import Rect
from arenbels.display.engine.spriteScheduler import SpriteScheduler
...
class SpriteNode(Node):
    ...
    def get_pos_camera(self,distorsion,box):
        ...
        pos_vect = box.translate2(trans).scale2(scale)
    ...
    def aff(self,fen,distorsion,dt):
        ...
        if self.sps is not None:
            if self.sps.loaded: #Check if it's loaded
                ...
                img = self.sps.get_sprite()
                #Get the image associated to the state of its SpriteScheduler

                #----------------------------------------
                #  Computes where to blit on the camera
                #----------------------------------------
                #Get the box in which this spriteNode needs to be drawn
                (px,py,pw,ph) = self.get_pos_camera(distorsion,self.get_size())
                #Check different types of mapping
                if self.mapping == "Flat":
                    #Extends the image
                    img = smoothscale(img,(int(pw),int(ph)))
                    fen.blit(img,(int(px*self.mult_offset)+self.x_offset ,int(py*self.mult_offset)+self.y_offset ))
                    ...
                    while dx < px+pw and ph != 0:
                        ...
                        img2 = smoothscale(img,(int(ratio*w)+1,int(ratio*h))) #Scales the image so that Y will fit
                    ...
                    logging.info("Unknown mapping type. cf SpriteNode.")


            else:
                logging.error("""
                Images should never be imported on-the-fly!

                The SpriteScheduler should be loaded before using it.
                Use create_sps(name) instead of set_sps(name).""")
                exit(0)
